                                        ...
                                        case 2:
                                                switch(data.type){
                                                        case 1:
                                                                ...
                                                                pthread_mutex_lock(&srv->mutex);
                                                                if(sinch[0] != BN || get_next_player(gm->board) != BO || data.uchar_1 > 8 || gm->board[data.uchar_1] != BN){
                                                                        pthread_mutex_unlock(&srv->mutex);
                                                                        /*
                                                                                Ход не принят
                                                                        */
                                        ...
                                        case 2:
                                                switch(data.type){
                                                        case 1:
                                                                ...
                                                                pthread_mutex_lock(&srv->mutex);
                                                                if(sinch[0] != BN || get_next_player(gm->board) != BX || data.uchar_1 > 8 || gm->board[data.uchar_1] != BN){
                                                                        pthread_mutex_unlock(&srv->mutex);
                                                                        /*
                                                                                Ход не принят
                                                                        */
...
void *server_games(void *arg)
{
        ...
        while(1){
                new_client = accept(srv->clients_socket, (struct sockaddr *) &addr, &addr_len);
                if(new_client <= 0){
                        ...
                        switch(data.dtype){
                                ...
                                switch(data.type){
                                        ...
                                        */
                                                ...
                                                i = data.uchar_2;
                                                pthread_mutex_lock(&srv->mutex);
                                                if( !(j > MAX_GAMES * 2 || i > MAX_GAMES) && srv->players[j] != NULL){
                                        ...
                                        if(srv->games[i] != NULL && srv->games[i]->is_run == FG_WAIT){
                                                ...
                                                if(j == -1){
                                                        ...
                                                        */
                                                        ...
                                                        new_client = 0;
                                                        PINF("client registered: %s:%d, %u(Pid), %u(Gid), X(smb), %s\n", inet_ntoa(addr.sin_addr), ntohs(addr.sin_port), j, i, data.char_buff);
