...
Server *server_init(char *ip, char *port_s, char *main_ip, char *main_port_s) {
  ...
  for (i = 0; i < MAX_GAMES; i++) srv->games[i] = NULL;
  for (i = 0; i < MAX_GAMES * 2; i++) srv->players[i] = NULL;
...
void create_logs_file() {
  ...
  logs = fopen(fname, "w");
  if (logs == NULL) {
    fprintf(stderr, "Error: create logs file %s\n", fname);
    exit(1);
  } else {
    pid = fork();
    if (pid == 0) {
