...
SLNode *server_find1_SLNode(Server *srv, int id)
{
        ...
        node = srv->servers;
        for(i = 0; i != id && i <= srv->servers_count; i++){
                node = node->right;
        }
        if(node == NULL){
...
/*
        ...
        Только для servers_pid
*/
void server_add_SLNode(Server *srv, char *ip, unsigned int s_port, unsigned int g_port)
{
        SLNode *node;
...
void *server_servers(void *arg)
{
        ...
        if(srv->main_server_socket > 0){
                ...
                sprintf(data.char_buff, "%s %u %u", srv->ip, srv->servers_port, srv->games_port);
                data.dtype = 1;
                data.type = 4;
        ...
        while(1){
                ...
                while(epl_count > 0){
                        ...
                        if(events[epl_count].data.fd == srv->servers_socket){
                                ...
                                */
                                if(new_server > 0)
                                        close(new_server);
                                addr_len = sizeof(struct sockaddr_in);
                                new_server = accept(srv->servers_socket, (struct sockaddr *) &addr, &addr_len);
                        ...
                        if(events[epl_count].data.fd == srv->main_server_socket){
                                ...
                                */
                                clear_Data(&data);
                                data_len = recv(srv->main_server_socket, &data, Data_size, MSG_NOSIGNAL);
                                if( data_len < 0 ){
                                                        ...
                                                        switch(data.uchar_1){
                                                                case 0:
                                                                        ...
                                                                        if(j == -1){
                                                                                ...
                                                                                pthread_mutex_lock(&srv->mutex);
                                                                                sscanf(data.char_buff, "%s %u", srv->players[j]->ip, &srv->players[j]->gid);
                                                                                srv->players[j]->symbol = data.uchar_3;
                                                                                if(srv->players[j]->gid >= 0 && srv->players[j]->gid < MAX_GAMES){
                                                        ...
                                                        switch(data.uchar_1){
                                                                case 1:
                                                                        ...
                                                                        */
                                                                        ...
                                                                        srv->main_server_socket = connect_to_main_server(char_buff, uint_1);
                                                                        if(!(srv->main_server_socket > 0))
                                                                                goto err_exit;
                                                                        sprintf(data.char_buff, "%s %u %u", srv->ip, srv->servers_port, srv->games_port);
                                                                        data.dtype = 1;
                                                                ...
                                                                case 1:
                                                                        ...
                                                                        */
                                                                        if(extra_server > 0){
                                                                                slnode = srv->servers;
                                                                                while(slnode != NULL){
                                                                                        sprintf(data.char_buff, "%s %u %u ", slnode->ip, slnode->s_port, slnode->g_port);
                                                                                        data.dtype = 1;
                                                                                        data.type = 10;
                                                                                        if( send(extra_server, &data, Data_size, 0) <= 0 ){
                                                                                                PERR("send data to extra_server\n");
                                                                        ...
                                                                        */
                                                                        for(j = 0; j < MAX_GAMES * 2; j++){
                                                                                ...
                                                                                if(srv->players[j] != NULL){
                                                                                        sprintf(data.char_buff, "%s %u", srv->players[j]->ip, srv->players[j]->gid);
                                ...
                                if(slnode != NULL){
                                        if(strcmp(srv->ip, slnode->ip) == 0
                                                ...
                                                /*
                                                        ...
                                                        значит упал последний сервер из списка, а его можно спокойно удалять
                                                */
                                                slnode = server_find1_SLNode(srv, srv->servers_count - 1);
                                                if(slnode != NULL){
                                                        if(srv->main_server_socket > 0){
                ...
                if((fl_err & EBMA) == EBMA){
                        ...
                        */
                        ...
                        if(srv->main_server_socket > 0){
                                ...
                                slnode = server_find2_SLNode(srv, srv->main_server_info.ip, srv->main_server_info.s_port);
                                if(slnode != NULL){
                                        server_push_Message(srv, 7, 0, slnode);
                                        slnode = slnode->left;
                                        while(slnode != NULL){
                                                PINF("connect to new main server %s:%u\n", slnode->ip, slnode->s_port);
                                                srv->main_server_socket = connect_to_main_server(slnode->ip, slnode->s_port);
                                                if(srv->main_server_socket > 0){
                                                        sprintf(data.char_buff, "%s %u %u ", srv->ip, srv->servers_port, srv->games_port);
                                                        data.dtype = 1;
                                                        data.type = 4;
                ...
                while(msg != NULL){
                        switch(msg->act){
                                ...
                                case 3:
                                        if(extra_server > 0){
                                                pthread_mutex_lock(&srv->mutex);
                                                sprintf(data.char_buff, "%s %u", srv->players[msg->uint]->ip, srv->players[msg->uint]->gid);
                                ...
                                case 6:
                                        ...
                                        break;
                                case 7:
                                        sprintf(data.char_buff, "%s %u %u ", msg->node->ip, msg->node->s_port, msg->node->g_port);
                                        data.dtype = 1;
                                        if(extra_server > 0){
                                                data.type = 11;
                                                if( send(extra_server, &data, Data_size, 0) <= 0 ){
